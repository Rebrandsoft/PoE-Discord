...
bot = BotWithReactions(command_prefix='-', description='PoE Info Powered By WildRage (D.K).')
...
class Alerts(commands.Cog):
    ...
    async def deals_filter(self, ctx, regexp=None):
        ...
        default (show all) is: .*'''
        if not regexp:
            r = bot.cursor.execute('SELECT regexp FROM regexp_filters WHERE channel=? AND type=?',
                                   (ctx.message.channel.id, 'dailydeal'))
            res = r.fetchone()
            await bot.send_message(ctx.message.channel, 'Current filter is: {}'.format(res[0] if res else '.*'))
        else:
            r = bot.cursor.execute('REPLACE INTO regexp_filters (channel,type,regexp) VALUES (?,?,?)',
                                   (ctx.message.channel.id, 'dailydeal', regexp))
            bot.conn.commit()
